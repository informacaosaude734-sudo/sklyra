...
export function PricingSection({
	...
	return (
		...
		>
			...
			/>
			<div className="mx-auto grid w-full max-w-6xl grid-cols-1 gap-6 md:gap-8 md:grid-cols-3">
				{plans.map((plan) => (
...
export function PricingCard({
	...
	return (
		<div
			key={plan.name}
			className={cn(
				'relative flex w-full flex-col rounded-lg border transition-all duration-300 hover:border-primary/30 hover:-translate-y-1',
				className,
			)}
			{...props}
		>
			...
			>
				<RouterLink
					...
					className={`w-full !px-6 !py-3 !text-sm ${
						plan.highlighted ? 'btn-framer' : 'btn-grain-outline'
					}`}
				>
					{plan.btn.text}
				</RouterLink>
